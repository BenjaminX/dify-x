...
## Repo map
- `index.html`: single bilingual home page (EN / 中文) — JS i18n with system-language detection, `L` shortcut, staggered flip-wave language switch, aria-live announcements, fixed two-pane layout (hero left, FLIP-filtered deck list scrolling right), Codrops-style motion (odometer counters, ⌘K quick-jump overlay with keyboard selection, `?` shortcuts help, ink-panel page transition into decks, j/k row navigation, hero title cursor parallax, cursor lamp, directional row reveals, per-letter hover wave, sweep + tilt + press-feedback hover, magnetic chips and asterisk, click-ripple particle canvas and buttons, elastic language pill, text-roll footer links, velocity-driven marquee with letter wave, live clock, ghost-number parallax, footer letter draw-in, scroll reveals). Decks live in the `DECKS` array in this file.
